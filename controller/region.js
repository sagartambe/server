...
module.exports = {
  ...
  create: (req, res) => {
    ...
    Region.create(input)
      ...
      .catch(err => {
        res.status(500).send({
          message: err.errors[0].message || 'Fail to create region'
        });
      });
  },
  update: (req, res) => {
    ...
    Region.update(input, {where: condition})
      ...
      .catch(err => {
        res.status(500).send({
          message: err.errors[0].message || 'Fail to update region'
        });
      });
  },
  delete: (req, res) => {
    ...
    Region.destroy({where: condition})
      .then(data => {
        if (data) {
          ...
          res.status(500).send({
            message: 'Fail to delete region'
          });
        }
      })
      .catch(err => {
        res.status(500).send({
          message: err.errors[0].message || 'Fail to delete region'
        });
      });
  }
}